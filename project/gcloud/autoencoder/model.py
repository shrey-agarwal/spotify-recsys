...
def MSEloss(inputs, targets, size_avarage=True):
    mask = targets != 0
    num_ratings = torch.sum(mask.float())
    criterion = nn.MSELoss(size_average=size_avarage)
    return criterion(inputs * mask.float(), targets), Variable(torch.Tensor([1.0]).cuda()) if size_avarage else num_ratings
    
def CEloss(inputs, targets, size_avarage=True):
    mask = targets != 0
    num_ratings = torch.sum(mask.float())
    criterion = nn.CrossEntropyLoss(size_average=size_avarage)
    return criterion(inputs * mask.float(), targets), Variable(torch.Tensor([1.0]).cuda()) if size_avarage else num_ratings
    
def CosLoss(inputs1, inputs2, targets, size_avarage=True):
    num_ratings = inputs1.shape[0]
    criterion = nn.CosineEmbeddingLoss(size_average=size_avarage)
    return criterion(inputs1, inputs2, targets), Variable(torch.Tensor([1.0]).cuda()) if size_avarage else num_ratings

def MyCosLoss(inputs, targets, size_avarage=True):
    mask = targets != 0
    num_ratings = torch.sum(mask.float())
    return torch.sum((1- F.cosine_similarity(inputs, targets)))/inputs.shape[0], Variable(torch.Tensor([1.0]).cuda()) if size_avarage else num_ratings
