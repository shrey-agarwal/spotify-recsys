...
parser.add_argument('--drop_prob', type=float, default=0.1, metavar='N',
...
def evaluate(encoder, evaluation_data_layer):
    ...
    for i, (eval, src) in enumerate(tqdm(evaluation_data_layer.iterate_one_epoch_test(pid_list))):
        inputs = Variable(src.cuda() if use_gpu else src)
        targets = Variable(eval.cuda() if use_gpu else eval)
...
def main():
    ...
    with open('v2/annoy-dim-256-tree-100-mincount-10/playlist_embeddings.pkl', 'rb') as f:
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, mb in enumerate(train_data_layer.iterate_one_epoch()):   #Work on this
            ...
            loss, num_ratings = model.CosLoss(outputs, inputs, torch.FloatTensor(np.ones((1, len(mb)))).cuda())
...
def test(infile, outfile):
    ...
    with open(infile, 'rb') as f:
        ...
        test_data_layer = input_v2.PlaySongRecDataProvider(test_data_layer, params)    #Use my own input data
 
    print("Data loaded")
    print("Vector dim: {}".format(test_data_layer.vector_dim))
    rencoder = model.AutoEncoder(layer_sizes=[test_data_layer.vector_dim] + [int(l) for l in args.hidden_layers.split(',')],
                           nl_type=args.non_linearity_type,
                           is_constrained=args.constrained,
                           dp_drop_prob=args.drop_prob,
                           last_layer_activations=not args.skip_last_layer_nl)
    ...
    rencoder.eval()
    for i, mb in enumerate(test_data_layer.iterate_test_epoch()):   #Work on this
        inputs = Variable(mb.cuda() if use_gpu else mb)
        outputs = rencoder(inputs)
...
if __name__ == '__main__':
    if(not args.test):
        ...
        test_metric = False
        if test_metric:
            infile = 'v2/my_chlng_plist_embed.pkl'
            outfile = '/challenge_output_embeddings.pkl'
        else:
            infile = 'v2/annoy-dim-256-tree-100-mincount-10/test_embeddings.pkl'
            outfile = '/output_embeddings.pkl'
        test(infile, outfile)
