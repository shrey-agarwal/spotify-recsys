...
def normalize_name(name):
    ...
    resultwords1  = [word for word in querywords if word.lower() not in ['music', 'songs', 'play list', 'play', 'song', 'list', 'hits', 'hit']]
...
def get_topN_playlists_helper(playlists, pid_list, topN=10):
    ...
    for i, playlist in enumerate(playlists):
        ...
        d = {}
        playlist = normalize_name(playlist)
        ...
        for name,score in topN:
            ...
            if len(temp_list) < 10:
                ...
                break
        top_songs[pid_list[i]] = temp_list[:10]
...
if __name__ == '__main__':
    ...
    playlists = ['country hits', 'pop', 'songs', 'rock', 'trap music']
    ps_ids = get_topN_playlists(playlists, 10, 4)
    for id in ps_ids:
        for i in ps_ids[id]: 
