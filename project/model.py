...
def CEloss(inputs, targets, size_avarage=False):
    mask = targets != 0
    num_ratings = torch.sum(mask.float())
...
def Cosloss(inputs1, inputs2, targets, size_avarage=False):
    ...
    return criterion(inputs1 * mask.float(), inputs2 * mask.float(), targets), Variable(torch.Tensor([1.0])) if size_avarage else num_ratings
    
def MyCosLoss(inputs, targets):
    return torch.sum((1- F.cosine_similarity(inputs, targets)))/inputs.shape[0]
